...
pub fn Rc(comptime T: type) type {
    ...
    return struct {
        ...
        ///
        pub fn init(alloc: Allocator, data: T) !@This() {
            ...
            if (is_slice) {
                ...
                const data_slice = try alloc.alloc(child_type, data.len);
                @memcpy(data_slice, data);
        ...
        pub fn manage(alloc: Allocator, data: if (is_slice) T else *T) !@This() {
            const ref_counter_ptr = try alloc.create(usize);
            errdefer alloc.destroy(ref_counter_ptr);
            ref_counter_ptr.* = 1;
            return .{ .ref_counter = ref_counter_ptr, .data = data };
        }

        pub fn aquire(self: *const @This()) @This() {
            self.ref_counter.* += 1;
